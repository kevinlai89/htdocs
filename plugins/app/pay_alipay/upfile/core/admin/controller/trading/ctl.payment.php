...
class ctl_payment extends objectPage {
    ...
    */
    ...
    function index(){
         ...
         // modify by zhangjunhua
         $cilent->url = constant('SDS_API');         
    ...
    */
    function savePayment(){
        ...
        if(isset($_POST['member_id']) && $_POST['member_id']!=""){
             $oPay = &$this->system->loadModel('trading/payment');
             $oPlu = $oPay->loadMethod($_POST['pay_type']);
             if(method_exists($oPlu,'alipayxml')){
                 $oPlu->alipayxml();
             }
        }

//selectarea(sel,this.value)
//sel.options[sel.selectedIndex].get('')
