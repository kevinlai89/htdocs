...
class installCore extends kernel{
    ...
    function __construct(){
        ...
        if(file_exists('../config/install.lock') && (!isset($_GET['step']) || ($_GET['step']!='active' && $_GET['step']!='complete'))){
    ...
    function step_complete(){
        $this->loadConfig();
         ...
         $admin_url='http://'.$_SERVER['HTTP_HOST'].dirname(dirname($_SERVER['PHP_SELF'])).'/shopadmin';
      $getent_sign=$certi_model->getent_sign();
      if(!$getent_sign){
        $this->pagedata['shop_url']=('http://'.$_SERVER['HTTP_HOST'].dirname(dirname($_SERVER['PHP_SELF']))); 
        $this->pagedata['shopadmin_url']=('http://'.$_SERVER['HTTP_HOST'].dirname(dirname($_SERVER['PHP_SELF'])).'/shopadmin');
        $this->pagedata['password'] = $_POST['password'];
    ...
    function step_active(){
      ...
      $getent_sign=$certi_model->getent_sign();
      if(!$getent_sign){
        $this->pagedata['link_url']=base64_encode('http://'.$_SERVER['HTTP_HOST'].dirname(dirname($_SERVER['PHP_SELF'])).'/shopadmin');
