...
class ctl_product extends shopPage{
    ...
    function ctl_product(){
        ...
        $this->default_view     = $this->system->getConf('gallery.default_view');
    }
    ...
    function index($gid,$specImg='',$spec_id='') {
        ...
        if (!empty($aPdtIds)) {
            $aPkgList = $oPackage->findPmtPkg($aPdtIds,true);
    ...
    */
    ...
    function _selector($params,&$smarty){
        if ($params['type']<>'b'){
            ...
            $args[4] = 6;
            if(!isset($smarty->_vars['curView'])){
                $smarty->_vars['curView'] = $this->default_view;
            }
