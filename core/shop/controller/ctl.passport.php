...
class ctl_passport extends shopPage{
    ...
    //信任登陆cookie设置
    function otherlogin_fastbuy(){
        $cart_model = $this->system->loadModel('trading/cart');
        $aParams = $cart_model->getParams($_POST['goods']);
        $cart =  $cart_model->setFastBuy('g', $aParams);
        setcookie('S[Cart_Fastbuy]', $cart_model->_save($cart));
    }
    ...
    function other_login_verify(){
        ...
        if($_COOKIE['Cart_Fastbuy']){
            $goods_info = explode("-",$_COOKIE['Cart_Fastbuy']);
             $product_info = explode("@",$goods_info[0]);

            $goods_id = substr($product_info[1],strpos($product_info[1],'.')+1);
            echo "<form action='?cart-checkout.html' method='post' id='fast_form'>
                   <input type='hidden' name='goods[product_id]' value='".$goods_info[1]."'/>
                   <input type='hidden' name='goods[num]' value='".$goods_info[3]."'/>

                   <input type='hidden' name='goods[pmt_id]' value=''/>

                   <input type='hidden' name='goods[goods_id]' value='".$goods_id."'/>

                   <input type='hidden' name='isfastbuy' value='1'/>

                  </form>
                  <script>var doms1 = document.getElementById('fast_form');doms1.submit();</script>";
            exit;
        }
    ...
    function login($url,$error_msg=''){
        ...
        $this->pagedata['ref_url'] = $this->deal_baseurl(base64_decode($url),1,false);
        ...
        if($error_msg!=''){
            $this->pagedata['err_msg'] = $this->deal_baseurl(base64_decode($error_msg),1);
        }
        $this->pagedata['options'] = $options;
        ...
        if ($common){
            ...
            if($this->system->getConf('site.register_valide')){
                if($_COOKIE["S_RANDOM_CODE"]!=md5($_POST['signupverifycode'])){
                    $this->splash('failed','back',__('验证码Error'),'','',$_POST['from_minipassport']);
                }
            }
    ...
    function nowredirect($status='success',$url,$msg="",$json=false){
        if($status!='failed'){
            ...
            if($json){
                ...
                $url = $this->system->mkUrl('passport','login',array($url,base64_encode($this->deal_baseurl($msg))));
                header('Location: '.$url);
                exit;
            }
        }
    ...
    function verify($passport='local')
    {
        ...
        if($this->system->getConf('site.login_valide')&&$passport=='local'){
            if($_COOKIE["L_RANDOM_CODE"]!=md5($_POST['loginverifycode'])){
                $this->nowredirect('failed',base64_encode($this->deal_baseurl($_POST['ref_url'])),__('验证码Error'),$_POST['from_minipassport']);
            }
        }
        $memberObj = &$this->system->loadModel('member/account');
        ...
        if ($obj=$pObj->function_judge('checkusername')){
            ...
            if ((is_array($uinfo)&&intval($uinfo[0])>0)||$info = $memberObj->verifyLogin($_POST['login'],$_POST['passwd'],$message)){
                ...
                $this->nowredirect('failed',base64_encode($this->deal_baseurl($_POST['ref_url'])),__('用户名OR密码有误 '),$_POST['from_minipassport']);
            }
        }
                //echo $_POST['ref_url'];exit;
        $this->title = '';
