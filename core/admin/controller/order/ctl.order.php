...
class ctl_order extends objectPage{
    //var $filter = array('order_refer'=>'local');
    ...
    var $finder_default_cols = '_cmd,order_id,order_refer,paipai_order_id,createtime,total_amount,ship_name,pay_status,ship_status,shipping,print_status,payment,member_id';
...
//echo $aItems[0]['nums'];exit();
    ...
    function showConsignFlow($orderid){
        ...
        $objOrder = &$this->system->loadModel('trading/order');
        $aShipping = $objOrder->getFieldById($orderid, array('order_id','paipai_order_id','ship_status','createtime','shipping_area','shipping_id','shipping','ship_name','is_delivery','ship_email','ship_tel','ship_mobile','ship_zip','ship_area','ship_addr','cost_freight','is_protect','cost_protect'));
        if(!$aShipping){
            echo __('发货错误：没有当前订单');
            return false;
        }
		...
		if($aShipping['paipai_order_id']!='0'){
            $this->pagedata['paipai_setting'] = 'disabled="disabled"';
		}
    ...
    function toDelivery($orderid){
        ...
        $_POST['opid'] = $this->system->op_id;
        $_POST['opname'] = $this->system->op_name;
        if($_POST['paipai_order_id']!=0){
			$paipai = $_POST;
			if($paipai['logi_no']==''){
			    $this->end(false, __('物流单号不能为空'));
			}
			$paipai['delivery_time'] = time();
            $this->paipai_todelivery($paipai);
			exit;
		}
		$objOrder = &$this->system->loadModel('trading/order');
    ...
    function paipai_todelivery($data){
		$objOrder = &$this->system->loadModel('trading/order');

        if($objOrder->paipai_delivery($data)){
            $connect_bind = &$this->system->loadModel('system/connect_bind');
            $res = $connect_bind->getNodetype('paipai');
			if($res >0 && $data['paipai_order_id']!=0){
               $matrix = &$this->system->loadModel('system/matrix');
               $oCorp = &$this->system->loadModel('trading/delivery');
               $aCorp = $oCorp->getCorpById($data['logi_id']);
			   $params['tid'] = $data['paipai_order_id']; 
               $params['logistics_no'] = $data['logi_no'];
               $params['logistics_code'] = $aCorp['type'];
			   $params['logistics_company'] = $aCorp['name'];
			   $params['method'] = "store.trade.delivery.send";
	           $params['event_name'] = "0";
			   $node_type = 'paipai';
			   $params['callback_url'] = $this->system->base_url()."shopadmin/index.php?ctl=receive_matrix&act=update_callback_status&type=store_trade_delivery_send";
               $matrix->insert_synclog($params,$node_type);
			} //添加拍拍发货 2011-11-24 add by yangminsheng
			$this->end(true, __('发货成功'));
		}
	}
...
//        $objPo->payByAfter($_POST['order_id'], $_POST['pay_id']);
     ...
     *@param $data['days'] 延长天数
     *@param certi_id 证书号
     *@param $from_node_id 我方node
     *@param $to_node_id 对方node
     *@date 2011-11-23
     */
    function paipai_receivetime_delay($orderid) {
      if(!$_POST['delay_time']){
		list($orderid,$delivery_id) = explode('t=',$orderid);
        $objOrder = &$this->system->loadModel('trading/order');
        $trade = $objOrder->getFieldById($orderid, array('paipai_order_id','delivery_time','delivery_delay_time'));
		$this->pagedata['delivery_time'] = date('Y-m-d H:i:s',$trade['delivery_time']);
		$return_day = $this->get_m_d_y((10*24*3600+$trade['delivery_time']+($trade['delivery_delay_time']*3600*24))-time());

        $this->pagedata['confirm_time'] = $return_day['day_time'];
        $this->pagedata['day'] = $return_day['day'];

		$this->pagedata['order_id'] = $orderid;
		$this->pagedata['paipai_order_id'] = $trade['paipai_order_id'];
		$this->display('order/receivetime_delay.html');
	  }else{
		$old_day = $_POST['old_day'];
		$delay_time = $_POST['delay_time'];
		$allow_time = array(3,5,7,10);
        $this->begin('index.php?ctl=order/order&act=detail&p[0]='.$_POST['order_id'].'&p[1]=detail_info');
		if(in_array($delay_time,$allow_time)){

			$db = $this->system->database();
			$deli_time = $db->selectrow('select delivery_delay_time from sdb_orders where paipai_order_id = "'.$_POST['paipai_order_id'].'"');
			$delivery_delay_time = $delay_time + $deli_time['delivery_delay_time'];

			$db->exec('update sdb_orders set delivery_delay_time = '.$delivery_delay_time.' where paipai_order_id = "'.$_POST['paipai_order_id'].'"');

			$matrix = &$this->system->loadModel('system/matrix');
			$params['tid'] = $_POST['paipai_order_id'];
			$params['days'] = $delay_time;
	        $params['event_name'] = "0";
			$node_type = 'paipai';
			$params['callback_url'] = $this->system->base_url()."shopadmin/index.php?ctl=receive_matrix&act=update_callback_status&type=store_trade_receivetime_delay";
			$params['method'] = "store.trade.receivetime.delay";
			$matrix->insert_synclog($params,$node_type);

		    $this->end(true,__('延长交易收货时间设定成功'));
		}else{
		    $this->end(false,__('参数有误,延长交易收货时间失败'));
		}
	  }
    }

	function paipai_delivery_send($orderid){
      $matrix = &$this->system->loadModel('system/matrix');
      $delivery = &$this->system->loadModel('trading/delivery');
      $aCorp = $delivery->fetch_delivery_info($orderid);
	  $db = $this->system->database();
	  $paipai_id = $db->selectrow('select paipai_order_id from sdb_orders where order_id = '.$orderid);
	  $params['tid'] = $paipai_id['paipai_order_id'];
	  $node_type = 'paipai';
      $params['logistics_no'] = $aCorp['logi_no'];
      $params['logistics_code'] = $aCorp['type'];
	  $params['logistics_company'] = $aCorp['name'];
	  $params['method'] = "store.trade.delivery.send";
	  $params['event_name'] = "0";
