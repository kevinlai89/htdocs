...
class ctl_messenger extends adminPage {
            ...
            if($_POST['targets']){
                ...
                if($sender == 'sms'){
                    foreach($member->getList($column,array('member_id'=>array_keys($_POST['targets'])),0,-1) as $info){
                        $info_name[] = $info;
                        $target = $_POST['targets'][$info['member_id']];
                    }
                }else{
                    foreach($member->getList($column,array('member_id'=>array_keys($_POST['targets'])),0,-1) as $info){
                ...
                if(isset($_POST['is_reply'])){
                    $info_name['use_reply'] = 1;
                }
                $messenger->addQueue($sender,$target,$_POST['title'],$info_name,$tmpl_name,5,MANUAL_SEND,'');
   ...
   function pagination($current,$totalPage,$param = 'queue',$method){ 
    ...
    function buySms(){
        ...
        //免登陆地址
        $this->pagedata['str_url'] = $oSms->getSmsUrl('sms','prdsList');
        $this->page('member/sms.html');
    }
    //短信转移 by zhangxuehui 2011-9-22
    function removeSms(){
        $oSms = $this->system->loadModel('system/sms');
        $url = $oSms->getSmsUrl('sms','accountsList');
        $url .= '&certi_id='.$this->system->getConf('certificate.id');
        $this->pagedata['str_url'] = $url;
        $this->page('member/sms.html');
    }
}
?>
