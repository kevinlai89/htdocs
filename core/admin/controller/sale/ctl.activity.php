...
class ctl_activity extends objectPage{
    ...
    function doActivityInfo($action) {
        ...
        if (empty($_POST['pmta_time_begin'])){
            $this->begin('index.php?ctl=sale/coupon&act=addCoupon');
            $this->end(false, '开始时间必选');
        }
        if (empty($_POST['pmta_time_end'])){
            $this->begin('index.php?ctl=sale/coupon&act=addCoupon');
            $this->end(false, '结束时间必选');
        }
        if ($_POST['pmta_time_begin'] >= $_POST['pmta_time_end']){
            $this->begin();
            $this->end(false, '结束时间必须大于开始时间','index.php?ctl=sale/activity&act=activityInfo');
        }
