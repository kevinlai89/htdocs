...
function bc_math($method,$left_operand,$right_operand,$scale=3){
    if(function_exists($method)){
        return $method(strval($left_operand),strval($right_operand),$scale);
    }
    else{
        $result = $GLOBALS['system']->database()->selectrow('SELECT '.$left_operand.'-'.$right_operand.' AS nums');
        if($result)
            return $result['nums'];
        else
