...
//$this->db->select('select * from sdb_goods where goods_id in( '.$gid.')');
    function getGoods($goods_id, $levelid=0){
        ...
        if($goods['pdt_desc'] = unserialize($goods['pdt_desc'])){    //如果是多物品商品
            ...
            asort($s);
            foreach($this->db->select('SELECT * FROM sdb_products WHERE product_id in ('.implode(',',$s).') AND goods_id='.$this->db->quote($goods_id).' ORDER BY product_id') as $product){
...
//                'cost'=>$pitem['cost'],
     ...
     //   $aData['uptime'] = time();
        ...
        if(is_array($products)){    //多物品商品
            ...
            foreach($products as $k => $aProduct){
                ...
                $maketable_label .= $aProduct['marketable'];
            ...
            $find = strpos($maketable_label,'true');
            if($find !==false &&$aData['marketable'] == 'true'){
                $aPdtDesc['marketable'] = 'true';
            }else{
                $aPdtDesc['marketable'] = 'false';
            }
    ...
    function updateGoodsIndex( $goods_id, $aData){
        ...
        foreach( $aData as $v ){
			...
			//防止spec_value_id和product_id重复插入导致报错
			if( $v['spec_value_id'] && $v['product_id'] ){
				$list = $this->db->selectrow('SELECT * FROM sdb_goods_spec_index WHERE spec_value_id = '.$this->db->quote($v['spec_value_id']).' AND product_id = '.$this->db->quote($v['product_id']));
				if(empty($list)){
					$aSql[] = ' ( '.implode(',', $v ).' ) ';
				}
			}
    ...
    function checkProductBn($bn, $gid=0){
        ...
        if($gid){
            $sql = 'SELECT count(*) AS num FROM sdb_products WHERE bn = \''.$bn.'\' AND goods_id != '.$this->db->quote($gid);
        }else{
    ...
    function toRemove($goodsid){
        ...
        foreach( $this->db->select('SELECT bn FROM sdb_products WHERE goods_id = '.$this->db->quote($goodsid)) as $prok => $prov ){
...
//----EVER checked here--------
    ...
    function getProducts($gid, $pid=0){
        $sqlWhere = '';
        if($pid > 0) $sqlWhere = ' AND A.product_id = '.intval($pid);
    ...
    function getFavorite($nMemberId,$nPage){
        $aGid = $this->db->selectrow('SELECT addon FROM sdb_members WHERE member_id='.$this->db->quote($nMemberId));
        if($aGid && $aGid['addon']!=''){
            $sSql = '';
...
//            break;
    ...
    function getKeywords($gid){
        return $this->db->select('SELECT keyword FROM sdb_goods_keywords WHERE goods_id = '.$this->db->quote($gid));
    }
    ...
    function updateUpDownTime($marketable,$goods_id,$goods_edit=false){
      ...
      if($marketable == 'true'){
         if($goods_edit==false){
            ...
            //取出goods_id 对应的上架时间
            $task_online = 'select tasktime from sdb_gtask where action = "online" and goods_id ='.$this->db->quote($goods_id).' ORDER BY tasktime';
            ...
            //取出goods_id 对应的下架时间
            $task_offline = 'select tasktime from sdb_gtask where action = "offline" and goods_id ='.$this->db->quote($goods_id).' ORDER BY tasktime';
    ...
    function getMarketableById($gid){
        return $this->db->selectrow('SELECT marketable FROM sdb_goods WHERE goods_id='.$this->db->quote($gid));
    }
}
?>
