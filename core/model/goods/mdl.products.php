...
class mdl_products extends shopObject{
    ...
    function getColumns($filter,$from = null){
        ...
        $this->_get_column_extend($ret);
        return $ret;
    }

    function _get_column_extend(&$cols) {
        $appmgrMdl =& $this->system->loadModel('system/appmgr');
        $radarApp =& $appmgrMdl->load('commodity_radar');
        if ( $radarApp->is_active ) {
            $cols['_commodity_radar'] = array(
                'label'=>__('商品雷达'),
                'noOrder'=>true,
                'hidden'=>false,
                'width'=>75,
                'locked'=>true,
                'html'=>PLUGIN_DIR.'/app/commodity_radar/view/finder_name.html',
            );
        }
    }
    ...
    function filter_getList($g){/*前台搜索商品规格筛选*/
        $sql='select goods_id from sdb_products where pdt_desc like \'%'.$g['pdt_desc'].'%\'and marketable=\'true\' and goods_id ='.$this->db->quote($g['goods_id']).'';
        return $this->db->select($sql);
    }

    function getList($cols,$filter='',$start=0,$limit=20,$orderType=null){
        if($_COOKIE['WEBSITE']=='mobile'){
            $limit = $this->system->getConf('app.website.page.listnum');
        }
    ...
    function wFilter($words){
        ...
        foreach($word as $k=>$v){
            if($v){
                $goodsId = array();
                foreach($oGoods->getGoodsIdByKeyword(array($v),'has') as $idv)
    ...
    function getTypeExportTitle(&$gtype){
        ...
        $col_title2['big_pic'] = __('col:大图片');
        $col_title2['small_pic'] = __('col:小图片');
        if($gtype['type_id']){
    ...
    function addSellLog($data){
        ...
        $orderData = $this->db->selectrow('SELECT o.member_id, m.uname,o.ship_email FROM sdb_orders o LEFT JOIN sdb_members m ON o.member_id = m.member_id WHERE o.order_id = '.$this->db->quote($data['order_id']));
        $orderItem = $this->db->select('SELECT p.price, p.goods_id, i.product_id, p.name,p.pdt_desc, i.nums FROM sdb_order_items i LEFT JOIN sdb_products p ON p.product_id = i.product_id WHERE i.order_id = '.$this->db->quote($data['order_id']));
