...
class mdl_specification extends shopObject{
    ...
    function getListByTypeId($typeid) {
        return $this->db->select('SELECT s.* FROM sdb_goods_type_spec ts INNER JOIN sdb_specification s ON ts.spec_id = s.spec_id WHERE s.disabled = "false" AND ts.type_id = '.$this->db->quote($typeid));
    }

    function getValueidByName($spec_id, $name){
        $sqlString = 'SELECT spec_value_id FROM sdb_spec_values Where spec_id = '.$this->db->quote($spec_id).' AND spec_value =\''.$name.'\'';
