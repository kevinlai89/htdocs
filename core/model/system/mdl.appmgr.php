...
class mdl_appmgr extends modelFactory{
   ...
   function login_refer($row){
    ...
    function restore_controller($ctl=null,$ident){
    ...
    function getList($nocompare=false){
        ...
        /*
        *$list 本地数据表里的app信息
        *$check_list 检测app目录后获取的app信息 
        */
        if($nocompare == 'local_update'){//实现app包本地化更新
        $file_app = &$this->system->loadModel('system/addons');
        $check_list = $file_app->_read_app_dir(0,PLUGIN_DIR.'/app');

           foreach($check_list as $key=>$value){
              if(substr($value['plugin_ident'],0,4) == 'pay_'){
                 unset($check_list[$key]);
                 continue;
              }
              $check_list_filter[] = $value['plugin_ident'];            
           }    
                                           
           foreach($list as $key=>$value){
                if(in_array($list[$key]['plugin_ident'],$check_list_filter)){
                      if($list[$key]['plugin_version'] < $check_list[$list[$key]['plugin_ident']]['plugin_ver']){
                           $list[$key]['plugin_ident'] = $check_list[$list[$key]['plugin_ident']]['plugin_ident'];
                           $list[$key]['local_update'] = true;
                      }
   ...
   function getUseapp($data){
            ...
            foreach($return as $key=>$val){
                if('pay_'.$pv['pay_type'] == $val['pay_ident']||$pv['pay_type']== $val['pay_ident']){
                    if($val['sort'] == '0'){
                        $alipay[$key] = $val;
                        $alipay[$key]['id'] = $pv['id'];
                        $alipay[$key]['custom_name'] = $pv['custom_name'];
                        $alipay[$key]['dis'] = $pv['disabled'];
                    }else{
                        $result[$pk] = $val;
                        $result[$pk]['id'] = $pv['id'];
        ...
        ksort($alipay);
        $total_result = array_merge_recursive($alipay,$result);
    ...
    function load($ident){
        ...
        if($data){
            ...
            if($data['plugin_base']=='0'){
                ...
                $obj->is_active = ('false' === $data['disabled']);
            }
            return $obj;
        }else{
            return false;
    ...
    function enable($ident){
        ...
        if(method_exists($app,'ctl_mapper')){
			foreach($app->ctl_mapper() as $k=>$v){

				$this->register_controller($k,$ident.':'.$v);
			}
		}
          if(method_exists($app,'crontab_queue')){
               foreach($app->crontab_queue() as $ek =>$ev){
                    $this->register_crontab_queue($ident.':'.$ev);
               }
          }
          if(method_exists($app,'custom_setting')){
                    $this->system->setConf('app.'.$ident.".custom_setting",$app->custom_setting());
          }
    ...
    function disable($ident){
        ...
        if($map){
            foreach($map as $k=>$v){
                $this->restore_controller($k,$ident);
            }
        }//else{
        //    $this->restore_controller(null,$ident);
       // }
          if(method_exists($app,'crontab_queue')){
    ...
    function openid_logo(){
        if($return = $this->db->select("SELECT * FROM sdb_plugins WHERE plugin_type='app' AND disabled='false' AND status ='used' AND plugin_ident LIKE 'openid%'")){
            return $return;
        }else{
            return false;
        }
    ...
    function ex_load($ex_type,$event_type){
        $task_dir = HOME_DIR.'/ext_task.xml';
		$xml = &$this->system->loadModel('utility/xml');
		$configs = file_get_contents($task_dir);
        $map =  $xml->xml2arrayValues($configs);
		$data = $map['ext_task']['serivce'];

		$objs = array();
		foreach($data as $k=>$v){
		   $_ex_type = $data[$k]['attr']['ex_type'];
		   $_event_type = $data[$k]['attr']['event_type'];
		   if($ex_type == $_ex_type && $event_type == $_event_type){
			   $objs['class'] = $data[$k]['class'];
		   }
		}
	    return $objs;
	}

	function loadComp($CompName){
        $comp_path = str_replace('_','/',$CompName);
		$dirname = dirname($comp_path);
        $filname = basename($comp_path);
		if(!class_exists($CompName)){
		   if(file_exists($path = CORE_DIR.'/component/'.$dirname.'/'.$filname.'.php')){
		      require_once($path);
		   }else{
			  trigger_error('class is not exist !',E_USER_ERROR);
		      return false;
		   }
		}
