...
//                        trigger_error(__('库存不足/购买数量超过限定数量/过期/超过最大购买限额'),E_USER_ERROR);
      ...
      if($trading['products']){
       foreach($trading['products'] as $rows){
        ...
        if(is_array($goods_id)){
            $goods_count = count(array_unique($goods_id));
        }else{
            $goods_count = 0;
        }
        $count = $goods_count+count($trading['gift_e'])+count($trading['package']);
        if($trading['products'])
            foreach($trading['products'] as $rows){
                $number += $rows['nums'];
            }
                    ...
                    case 'g'://商品
                        ...
                        if(!empty($sCart)){
                            ...
                            foreach($aRow as $sRow){
                                $aTmp = explode("-", $sRow);
                                $aCart['g']['cart'][$aTmp[0].'-'.$aTmp[1].'-'.$aTmp[2]] = intval($aTmp[3]);
                            }
                            $aRow = explode(",", $sPmt);
                            foreach($aRow as $sRow){
                                $aTmp = explode("-", $sRow);
    ...
    function md5_cart_objects($aCart=array()){
        if(!$aCart){
            $aCart = $this->getCart("all");
        }
        return $this->array_md5($aCart);
    }

    function array2str_recursive($data, &$returnStr) {
        if(is_array($data)){
            ksort($data);
            foreach($data AS $k=>$v){
                $returnStr .= $k;
                $this->array2str_recursive($v, $returnStr);
            }
        }
        else{
            $returnStr .= $data;
    ...
    function array_md5($array){
        return $this->array2str_recursive($array, $str);
    }
}
?>